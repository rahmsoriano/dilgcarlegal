<x-admin-layout>
    <div class="max-w-4xl mx-auto space-y-8">
        ...
        <div class="flex items-center gap-6">
            ...
            <div>
                <h2 class="text-3xl font-bold text-white tracking-tight">Upload Law</h2>
                <p class="mt-1 text-slate-400">Add a new statute or legal reference to the library.</p>
            </div>
        </div>
        ...
        <div class="rounded-[2.5rem] bg-white/[0.02] backdrop-blur-xl p-10 ring-1 ring-white/10 shadow-2xl">
            <form action="{{ route('admin.laws.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                
                <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold uppercase tracking-widest text-slate-500 mb-3">Law Title</label>
                        <input type="text" name="title" value="{{ old('title') }}" placeholder="e.g., Ecological Solid Waste Management Act of 2000" required
                    ...
                    <div>
                        ...
                        <input type="text" name="law_number" value="{{ old('law_number') }}" placeholder="e.g., RA 9003" required
                            class="w-full rounded-2xl bg-white/[0.03] border-white/10 px-6 py-4 text-sm text-white placeholder:text-slate-600 focus:border-blue-500/50 focus:ring-blue-500/20 transition-all">
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-widest text-slate-500 mb-3">Category</label>
                        <input type="text" name="category" value="{{ old('category') }}" placeholder="e.g., Environment, Local Government"
                            class="w-full rounded-2xl bg-white/[0.03] border-white/10 px-6 py-4 text-sm text-white placeholder:text-slate-600 focus:border-blue-500/50 focus:ring-blue-500/20 transition-all">
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-widest text-slate-500 mb-3">Year</label>
                        <input type="number" name="year" value="{{ old('year', date('Y')) }}" placeholder="e.g., 2000" required
                            class="w-full rounded-2xl bg-white/[0.03] border-white/10 px-6 py-4 text-sm text-white placeholder:text-slate-600 focus:border-blue-500/50 focus:ring-blue-500/20 transition-all">
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-widest text-slate-500 mb-3">Law Document</label>
                        ...
                        <p class="mt-2 text-[10px] text-slate-600 uppercase tracking-widest font-bold">PDF, DOCX, or TXT (Max 10MB)</p>
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold uppercase tracking-widest text-slate-500 mb-3">Brief Description</label>
                        <textarea name="description" rows="4" placeholder="Brief summary of what this law covers..."
                            class="w-full rounded-2xl bg-white/[0.03] border-white/10 px-6 py-4 text-sm text-white placeholder:text-slate-600 focus:border-blue-500/50 focus:ring-blue-500/20 transition-all resize-none">{{ old('description') }}</textarea>
                    </div>
                </div>
