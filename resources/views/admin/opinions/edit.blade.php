<x-admin-layout>
    <div class="max-w-5xl mx-auto space-y-8">
        <div class="flex items-center gap-6">
            <a href="{{ route('admin.opinions.show', $opinion) }}" class="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/[0.03] text-slate-400 ring-1 ring-white/10 hover:bg-white/[0.05] hover:text-white transition-all duration-300">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="h-5 w-5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
                </svg>
            </a>
            <div>
                <h2 class="text-3xl font-bold text-white tracking-tight">Edit Legal Opinion</h2>
                <p class="mt-1 text-slate-400">Update an existing legal reference entry.</p>
            </div>
        </div>
        ...
        <div class="rounded-[2.5rem] bg-white/[0.02] backdrop-blur-xl p-10 ring-1 ring-white/10 shadow-2xl">
            <form action="{{ route('admin.opinions.update', $opinion) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-8">
                    ...
                    <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="date" name="date" value="{{ old('date', optional($opinion->date)->format('Y-m-d')) }}" required
                                class="w-full rounded-2xl bg-white/[0.03] border-white/10 px-6 py-4 text-sm text-white placeholder:text-slate-600 focus:border-blue-500/50 focus:ring-blue-500/20 transition-all">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="context" rows="14" required
                            class="w-full rounded-2xl bg-white/[0.03] border-white/10 px-6 py-4 text-sm text-white placeholder:text-slate-600 focus:border-blue-500/50 focus:ring-blue-500/20 transition-all resize-y">{{ old('context', $opinion->context) }}</textarea>
                    </div>
                </div>

                <div class="flex items-center justify-end gap-4 pt-6 border-t border-white/5">
                    <a href="{{ route('admin.opinions.show', $opinion) }}" class="px-8 py-4 text-sm font-bold text-slate-400 hover:text-white transition-colors">Cancel</a>
